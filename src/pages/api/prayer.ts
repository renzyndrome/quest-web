...
// Prayer payloads are tiny (name + location + <=5000-char request). Cap the
// body BEFORE parsing so a giant payload can't buffer into memory ahead of the
// rate limiter (parse-first DoS). Bounds memory regardless of Content-Length.
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  try {
    input = JSON.parse(raw) as PrayerInput;
  } catch {
    return json({ ok: false, error: 'Invalid request.' }, 400);
  }

  const result = validatePrayer(input);
  ...
  try {
    const sent = await sendPrayerEmail(config, result.clean);
    if (!sent.ok) return json({ ok: false, error: 'send_failed' }, 502);
    // Note: no request contents are logged here, by design.
